...
import { getEntities } from './my-entity.reducer';
...
export const MyEntity = () => {
  ...
  const myEntityList = useAppSelector(state => state.myEntity.entities);
  const loading = useAppSelector(state => state.myEntity.loading);
  const totalItems = useAppSelector(state => state.myEntity.totalItems);
  ...
  return (
    <div>
      ...
      <div className="table-responsive">
        {myEntityList && myEntityList.length > 0 ? (
          <Table responsive>
            ...
            <tbody>
              {myEntityList.map((myEntity, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/my-entity/${myEntity.id}`} color="link" size="sm">
                      {myEntity.id}
                    </Button>
                  </td>
                  <td>{myEntity.name}</td>
                  ...
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/my-entity/${myEntity.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/my-entity/${myEntity.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
      ...
      {totalItems ? (
        <div className={myEntityList && myEntityList.length > 0 ? '' : 'd-none'}>
